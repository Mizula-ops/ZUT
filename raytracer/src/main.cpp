...
int main (int argc, char * const argv[]) {
    ...
    for (int j=0;j<scene.cam.height; j++) {
        for (int i=0;i<scene.cam.width; i++) {
            ...
            rgb.y = (ray.dir.y + 1.0f) / 2.0f; // Normalizowanie y do zakresu <0, 1>
            image_y.setPixel(i, j, rgb);

            rgb = glm::vec3(0.0f, 0.0f, 1.0f);
            rgb.z = (ray.dir.z + 1.0f) / 2.0f; // Normalizowanie z do zakresu <0, 1>
            image_z.setPixel(i, j, rgb);
        }
    }
    image_x.save("images/gradient-OX.png", true);
    image_y.save("images/gradient-OY.png", true);
    image_z.save("images/gradient-OZ.png", true);
    ...
    */
    ...
    image.save("images/intersection.png", true);
