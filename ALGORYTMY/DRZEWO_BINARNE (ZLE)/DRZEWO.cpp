//ALGO2 IS1 211B LAB03
//Artur Mizuła
//mizula.studio@gmail.com
#include <iostream>
#include <string>
#include <vector>
#include <cmath>  
...
some_object* generate_some_object()
{
    ...
    so->field_1 = ((rand() << 15) + rand()) % 1500;
    //so->field_2 = 'a' + (rand() % 26);
    so->field_2 = 'a';
    return so;
}
some_object* generate_some_object1()
{
    some_object* so = new some_object();
...
struct Node {
    ...
    int indeks=0;
    T data;
 Node(T data) : left(nullptr), right(nullptr), parent(nullptr), data(data), indeks(0) {}
};
    ...
    void add(T data, int(*cmp)(T, T)) {
        ...
        if (!root)
        {
            ...
            while (true) {
                ...
                else if (cmp(current->data, p->data)<0) {
                    if (p->left == nullptr) {
                        p->left = current;
                        current->parent = p;
                        size++;
    ...
    void print_helper(Node<T>* p, string& krokiet)
{
    if (p != nullptr)
    {
        ...
        krokiet += "(" + to_string(p->indeks) + ": [p: " + parent+ ", l: " + left+ ", r: " + right + "], data: " + to_string(p->data) + ")\n";
    ...
    void print_helper(Node<T>* p, string& krokiet, std::string(*see)(T),int& ile)
    {
        ...
        if (p != nullptr && ile < 10)
        {
            ...
            krokiet += "(" + to_string(p->indeks) + ": [p: " + parent + ", l: " + left + ", r: " + right + "], data: " + see(p->data) + ")\n";
            ile++;
...
int main1() {
    ...
    brat.add(33);
    cout<<brat.print1()<<endl;
    cout << brat.inorder()<<endl;
    cout << brat.preorder()<<endl;
    
    return 0;
}
int main() {
    const int MAX_ORDER = 7; 
    Tree < some_object* >* bst = new Tree < some_object*>(); 
        ...
        for (int o = 1; o <= MAX_ORDER; o++)
        {
            ...
            {
                some_object* so = generate_some_object();
                Node< some_object* >* result = bst-> find(so, some_objects_cmp);
                if (result != NULL)
                    hits++;
                delete so;
            }
            ...
            cout << bst->print1(see);
            float h = bst->height();
            float suma = h / log2(bst->size + 1);
            cout << "Wysokosc: " << h << endl;
                cout << "Rozmiar: " << bst->size << endl;
