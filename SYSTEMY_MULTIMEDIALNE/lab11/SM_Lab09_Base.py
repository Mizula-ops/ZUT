...
def plotDiffrence(ReferenceFrame,DecompressedFrame,ROI):
    ...
    axs[1].imshow(DecompressedFrame[ROI[0]:ROI[1],ROI[2]:ROI[3]])
    axs[2].imshow(diff, cmap="gray", vmin=0, vmax=10)
    for ax in axs:
        ax.set_axis_off()
            ...
            for i in range(ile):
                ...
                if np.any(plot_frames==i): # rysuj wykresy
                    for r in ROI_by_file[plik]:
                        fig = plotDiffrence(frame_RGB,decomp_RGB,r)
