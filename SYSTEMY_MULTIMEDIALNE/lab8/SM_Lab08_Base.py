...
def plot_comparisone(counter, OG,Decomp,JPEG,figsize=(5,8) ):
    ...
    before_y = PRZED_YCrCb[:, :, 0].size
    before_cr = PRZED_YCrCb[:, :, 1].size
    before_cb = PRZED_YCrCb[:, :, 2].size

    after_y = len(JPEG.Y)
    after_cr = len(JPEG.Cr)
    after_cb = len(JPEG.Cb)
    ...
    axs[1,1].set_title(f"Y procent {round(after_y/before_y*100,1)} %")
    axs[2,1].set_title(f"Cr procent {round(after_cr/before_cb*100,1)} %")
    axs[3,1].set_title(f"Cb procent {round(after_cb/before_cr*100,1)} %")
        ...
        for ROI in file_dict['ROIs']:
            ...
            for Chroma in Chroma_options:
                for Quant in Quant_options:
                    ...
                    f = plot_comparisone(Counter,fragment,New_Fragment,tJPEG)
