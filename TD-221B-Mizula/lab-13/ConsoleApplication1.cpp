...
int main1() {
    ...
    vector<double> betas = { 0.0,25,50,75,100,125,150,175,190,200 };
    ...
    for (double alpha : alphas) {
        ...
        for (double beta : betas) {
            ...
            generateFilter(noisySignalASK15, beta);
            generateFilter(noisySignalPSK15, 0.7 * beta);
            generateFilter(noisySignalFSK15, 5 * beta);

            generateFilter(noisySignalASK7, beta);
            generateFilter(noisySignalPSK7, beta);
            generateFilter(noisySignalFSK7, 2.5 * beta);
    ...
    cout << "Alpha\tBeta\tBER_ASK15\tBER_PSK15\tBER_FSK15\n";
    size_t index = 0;
    for (double alpha : alphas) {
        for (double beta : betas) {
            cout << alpha << "\t" << beta << "\t"
                << BER_ASK15[index].second << "\t"
                << BER_PSK15[index].second << "\t"
                << BER_FSK15[index].second << "\n";
            ++index;
        }
    }

    cout << "\nAlpha\tBeta\tBER_ASK7\tBER_PSK7\tBER_FSK7\n";
    index = 0;
    for (double alpha : alphas) {
        for (double beta : betas) {
            cout << alpha << "\t" << beta << "\t"
                << BER_ASK7[index].second << "\t"
                << BER_PSK7[index].second << "\t"
                << BER_FSK7[index].second << "\n";
            ++index;
        }
    }
    ofstream out("ber_results.txt");
    out << "alpha\tbeta\tber_ask15\tber_psk15\tber_fsk15\tber_ask7\tber_psk7\tber_fsk7\n";

    for (size_t i = 0; i < alphas.size(); ++i) {
        for (size_t j = 0; j < betas.size(); ++j) {
            size_t index = i * betas.size() + j;

            out << alphas[i] << "\t" << betas[j] << "\t"
                << BER_ASK15[index].second << "\t"
                << BER_PSK15[index].second << "\t"
                << BER_FSK15[index].second << "\t"
                << BER_ASK7[index].second << "\t"
